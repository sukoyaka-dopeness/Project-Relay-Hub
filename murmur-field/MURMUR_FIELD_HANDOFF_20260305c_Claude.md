...
| 2026-03-05 | 機能説明はオンボーディングで一括説明ではなく「初めてその操作に触れた瞬間に一度だけ」方式に統一 |
| 2026-03-05 | 久しぶりにログインした際は石カードを優先表示し、枯れ草は間引いて表示する方向で検討 |
| 2026-03-05 | 石の上限数：3〜5個程度が候補（「本当に大切なもの」の認知的上限に基づく。要検証） |
| 2026-03-05 | オンボーディング猶予期間30日は仮置き。7日・14日・30日の三段階でベータ検証を推奨 |
| 2026-03-05 | リンク切れ問題：単純な404に加え、ドメイン乗っ取り・スパム転生パターンを認識。MVPでは「疑わしいURL」マーク表示＋ユーザー判断委任を検討 |
| 2026-03-05 | 石を置いたカードのページアーカイブを将来的に検討（石カードのみ選択的保存。ストレージコストの現実解として） |
| 2026-03-05 | PHILOSOPHYファイルの新規作成を検討：「声を上げられない人」の問いを未解決として明記する |
...
- Stone card upper limit — candidate 3–5 (requires beta verification) / 石カードの上限数——3〜5個候補（要検証）
- MURMUR_FIELD_PHILOSOPHY.md — new document to be created / PHILOSOPHYファイルの新規作成
...
- Suspicious URL handling — spam-hijacked domain detection and display policy for MVP / 疑わしいURL（スパム転生ドメイン）の検出と表示方針——MVP向け
- Withered grass display count adjustment on「思い出す」/ 「思い出す」時の枯れ草表示数の調整方針
- Onboarding grace period optimal length — beta verification with 7/14/30 day variants / オンボーディング猶予期間の最適値——7日・14日・30日でベータ検証
...
- Page archiving for stone cards — storage cost consideration / 石カードのページアーカイブ（ストレージコスト検討）
